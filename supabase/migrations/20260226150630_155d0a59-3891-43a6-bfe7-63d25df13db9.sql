...
-- Add quiz fields to furore_sessions
ALTER TABLE public.furore_sessions
  ADD COLUMN IF NOT EXISTS quiz_question_id uuid DEFAULT null,
  ADD COLUMN IF NOT EXISTS quiz_answer_revealed boolean DEFAULT false;

-- Update the furore_update_session function to handle new fields
...
BEGIN
  ...
  SET 
    ...
    quiz_question_id = CASE 
      WHEN p_updates ? 'quiz_question_id' THEN (p_updates->>'quiz_question_id')::uuid
      ELSE quiz_question_id
    END,
    quiz_answer_revealed = COALESCE((p_updates->>'quiz_answer_revealed')::boolean, quiz_answer_revealed),
    updated_at = now()
