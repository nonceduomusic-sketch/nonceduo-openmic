...
import { Building2, PartyPopper, Heart, ArrowRight, Music, Sparkles, MapPin, Home } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background">
        {/* Header - Professional with Home button */}
        <header className="sticky top-0 z-50 border-b border-border/40 bg-background/95 backdrop-blur-xl shadow-sm">
          <div className="container mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Link 
                to="/" 
                className="w-9 h-9 rounded-full bg-card border border-border/60 flex items-center justify-center hover:bg-accent/10 hover:border-primary/40 transition-all duration-300 group"
                title="Torna al sito"
              >
                <Home className="w-4 h-4 text-muted-foreground group-hover:text-primary transition-colors" />
              </Link>
              <Link to="/" className="hover:opacity-80 transition-opacity">
                <img src={brandLogo} alt="Non c'è Duo" className="h-7 md:h-8 w-auto" />
              </Link>
            </div>
