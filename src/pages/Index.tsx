...
const Index: React.FC = () => {
  ...
  const filteredSongs = useMemo(() => {
    ...
    return songs.filter((song) => {
      ...
      const matchesSearch =
        song.title.toLowerCase().includes(searchLower) || song.artist.toLowerCase().includes(searchLower);
        ...
        <div className="container py-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display text-xl md:text-2xl font-bold neon-text-pink">Non C'è Duo</h1>
                <p className="text-xs md:text-sm text-secondary font-medium">Open Mic</p>
              </div>
            </div>

            {isStaff && (
              <Link to="/admin" className="p-2 rounded-lg bg-muted hover:bg-muted/80 transition-colors" title="Admin">
      ...
      <main className="container py-4 pb-8">
        ...
        <div className="mb-4 p-3 rounded-lg bg-muted/30 border border-border">
          <p className="text-sm text-muted-foreground text-center leading-relaxed">
            ...
            <br />
            👉 Clicca su <strong className="text-primary">Prenota</strong> per inviarci la richiesta su WhatsApp.
            <br />
