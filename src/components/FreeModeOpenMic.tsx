...
export const FreeModeOpenMic: React.FC<FreeModeOpenMicProps> = ({ freeModeState }) => {
  ...
  const filteredSongs = useMemo(() => {
    return songs.filter((song) => {
      ...
      const completed = isSongCompleted(song.title, song.artist);

      if (!searchLower) {
        return matchesArtist && !completed;
      }
      ...
      return matchesSearch && matchesArtist && !completed;
    });
  ...
  const bookedCount = queueSongs.length;
  ...
  return (
    <>
      <SEO
        ...
        description="Prenota la tua canzone per l'evento live!"
      />
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50">
          <div className="container mx-auto px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button variant="ghost" size="sm" className="relative" onClick={() => setShowQueue(!showQueue)}>
              ...
              {isReopenValid && (
                <div className="mt-3 pt-3 border-t-2 border-secondary/50 animate-pulse">
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-secondary/20 border border-secondary/40">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-bold text-secondary">🎉 {reopenMessage || 'Riapertura straordinaria!'}</p>
                      <p className="text-xs text-secondary/80">Affrettati! Posti extra disponibili per poco tempo</p>
                    </div>
                    {reopenRemaining && (
          ...
          {isClosed ? null : (
            <>
              <div className="space-y-3">
                <SearchBar value={search} onChange={setSearch} placeholder="Cerca canzone o artista..." />
                    ...
                    <CollapsibleTrigger asChild>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs bg-secondary/20 text-secondary px-2 py-0.5 rounded-full">{queueSongs.length} in coda</span>
                        </div>
              ...
              {loading ? (
                <div className="text-center py-12">
                  <Music2 className="w-12 h-12 text-primary animate-pulse mx-auto mb-3" />
        ...
        {selectedSong && !isClosed && (
          <BookingConfirmationModal song={selectedSong} onClose={() => setSelectedSong(null)} />
        )}
      </div>
    </>
  );
