          ...
          <Tabs value={activeSection} onValueChange={(v) => setActiveSection(v as typeof activeSection)}>
            ...
            <TabsContent value="pin" className="mt-4 space-y-4">
              <PinProtectionCard
                ...
                freeModeConfig={{
                  id: settings?.id ?? 'free-mode',
                  isActive: Boolean(settings?.is_active),
                  pinEnabled: Boolean(settings?.pin_enabled),
                  pinCode: settings?.pin_code ?? null,
                  showPinOnGate: Boolean(settings?.show_pin_on_gate),
                }}
                onUpdateFreeModeConfig={async (updates) => {
                  return updateSettings(updates as Partial<FreeModeSettings>);
                }}
              />
            </TabsContent>
...
