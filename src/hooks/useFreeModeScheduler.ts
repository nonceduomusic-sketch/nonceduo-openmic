...
export const useFreeModeScheduler = () => {
  ...
  // Usa solo expires_at come dipendenza per evitare re-run eccessivi
  const expiresAtStr = settings?.expires_at;
  const isActiveFlag = settings?.is_active;
  const endModeFlag = settings?.end_mode;
  
  useEffect(() => {
    if (loading || !isActiveFlag) return;
    
    // Se end_mode è manual, non fare nulla
    if (endModeFlag === 'manual') return;
    
    if (!expiresAtStr) return;
    
    const scheduledEnd = new Date(expiresAtStr);
    ...
    // Programma la chiusura - LOG solo una volta per scheduling
    console.log(`[FreeModeScheduler] Scheduled auto-end at ${scheduledEnd.toLocaleTimeString()} (in ${Math.round(msUntilEnd / 1000 / 60)} minutes)`);
  ...
  }, [loading, isActiveFlag, endModeFlag, expiresAtStr, deactivateFreeMode]);
